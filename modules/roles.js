...
async function handleRole(ctx, args, client) {
    ...
    if (sub === 'humans') {
        ...
        if (!role) return ctx.reply({ content: '❌ Mention a role. Usage: `.role humans add @role`' });
    ...
    if (sub === 'bots') {
        const action = args[1]?.toLowerCase();
        if (!role) return ctx.reply({ content: '❌ Mention a role. Usage: `.role bots add @role`' });
    ...
    if (sub === 'edit' && role) {
        ...
        if (!newName) return ctx.reply({ content: '❌ Provide a new name: `.role edit @role <name>`' });
        const old = role.name;
    ...
    if (sub === 'color' && args[1] !== 'gradient') {
        if (!ctx.member.permissions.has(PermissionFlagsBits.ManageRoles))
            return ctx.reply({ content: '❌ Missing `Manage Roles` permission.' });
        if (!role) return ctx.reply({ content: '❌ Mention a role: `.role color @role #hex`' });
    ...
    if (sub === 'color' && args[1] === 'gradient') {
        if (!ctx.member.permissions.has(PermissionFlagsBits.ManageRoles))
            return ctx.reply({ content: '❌ Missing `Manage Roles` permission.' });
        if (!role) return ctx.reply({ content: '❌ Mention a role: `.role color gradient @role #hex1 #hex2`' });
    ...
    // ── .role icon @role <url or attachment> ──
    if (sub === 'icon') {
        if (!ctx.member.permissions.has(PermissionFlagsBits.ManageRoles))
            return ctx.reply({ content: '❌ Missing `Manage Roles` permission.' });
        if (!role) return ctx.reply({ content: '❌ Mention a role: `.role icon @role <image URL or attach image>`' });
        ...
        if (!url) return ctx.reply({ content: '❌ Provide an image URL or attach an image.' });

        await role.setIcon(url);
        return ctx.reply({ embeds: [base(COLORS.success).setTitle('🖼️ Role Icon Updated')
            .addFields({ name: '🎭 Role', value: `<@&${role.id}>`, inline: true })] });
    }

    // ── .role mentionable @role ──
    if (sub === 'mentionable' && role) {
        if (!ctx.member.permissions.has(PermissionFlagsBits.ManageRoles))
            return ctx.reply({ content: '❌ Missing `Manage Roles` permission.' });
    ...
    return ctx.reply({ embeds: [base(COLORS.primary).setTitle('🎭 Role Commands')
        .setDescription([
            ...
            '`.role edit @role <name>` `.role color @role #hex`',
            '`.role color gradient @role #hex1 #hex2`',
