...
async function handleModerationCommand(ctx, command, args, client) {
    ...
    if (command === 'kick') {
        ...
        try {
            ...
            const invMsg = getInvokeReply(guild.id, 'kick', {
                targetMention: `<@${target.id}>`, targetName: target.user.username, targetId: target.id,
                modMention: `<@${authorId}>`, modName: ctx.member?.user?.username ?? authorId,
                reason, guildName: guild.name, guildId: guild.id,
            });
    ...
    if (command === 'ban') {
        ...
        const targetMember = target.user ? target : null; // might be just a User if fetched by ID
        const reason     = args.slice(1).join(' ').replace(/<@!?\d+>/g, '').trim() || 'No reason provided';
        ...
        try {
            const { sendInvokeDm, getInvokeReply } = require('./invoke');
            ...
            await sendModLog(guild, logEmbed);
            const invMsg = getInvokeReply(guild.id, 'ban', {
                targetMention: `<@${targetUser.id}>`, targetName: targetUser.username, targetId: targetUser.id,
                modMention: `<@${authorId}>`, modName: ctx.member?.user?.username ?? authorId,
                reason, guildName: guild.name, guildId: guild.id,
            });
            const reply = invMsg || { content: '👍', components: [] };
            return ctx.editReply ? ctx.editReply(reply) : ctx.reply(reply);
    ...
    if (command === 'softban') {
        ...
        try {
            ...
            await sendModLog(guild, logEmbed);
            const invMsg = getInvokeReply(guild.id, 'softban', {
    ...
    if (command === 'hardban') {
        ...
        try {
            ...
            await sendModLog(guild, logEmbed);
            const invMsg = getInvokeReply(guild.id, 'hardban', {
    ...
    if (command === 'tempban') {
        ...
        // Find duration arg (e.g., 1d, 2h, 30m) — skip the mention
        ...
        try {
            try { await target.user.send({ embeds: [base(COLORS.error).setTitle('⏱️ You were temporarily banned').addFields({ name: 'Server', value: guild.name, inline: true }, { name: 'Duration', value: formatDuration(duration), inline: true }, { name: 'Reason', value: reason })] }); } catch {}
            ...
            await sendModLog(guild, logEmbed);
            const { getInvokeReply } = require('./invoke');
            const invMsg = getInvokeReply(guild.id, 'tempban', {
                targetName: target.user.username, targetId: target.id, targetMention: `<@${target.id}>`,
    ...
    if (command === 'unban') {
        ...
        try {
            ...
            await sendModLog(guild, logEmbed);
            const { getInvokeReply } = require('./invoke');
            const invMsg = getInvokeReply(guild.id, 'unban', {
    ...
    if (command === 'unbanall') {
        ...
        return msg?.edit({ content: '', embeds: [base(COLORS.success).setTitle('🔓 Unbanall Complete')
            .addFields({ name: '✅ Unbanned', value: done.toString(), inline: true }, { name: '❌ Failed', value: failed.toString(), inline: true })] }).catch(() => {});
    }
    ...
    if (command === 'timeout') {
        ...
        try {
            ...
            await sendModLog(guild, logEmbed);
            const { getInvokeReply } = require('./invoke');
            const invMsg = getInvokeReply(guild.id, 'timeout', {
                targetName: target.user.username, targetId: target.id, targetMention: `<@${target.id}>`,
                modMention: `<@${authorId}>`, reason, duration: formatDuration(duration),
                guildName: guild.name, guildId: guild.id,
            });
    ...
    if (command === 'untimeout') {
        ...
        try {
            ...
            await sendModLog(guild, logEmbed);
            const { getInvokeReply } = require('./invoke');
            const invMsg = getInvokeReply(guild.id, 'untimeout', {
                targetName: target.user.username, targetId: target.id, targetMention: `<@${target.id}>`,
                modMention: `<@${authorId}>`, reason, guildName: guild.name, guildId: guild.id,
            });
            return ctx.reply(invMsg || '👍');
    ...
    if (command === 'warn') {
        ...
        // DM the user
        try { await target.user.send({ embeds: [base(COLORS.warning).setTitle('⚠️ You received a warning').addFields({ name: 'Server', value: guild.name, inline: true }, { name: 'Reason', value: reason }, { name: 'Total Warnings', value: total.toString(), inline: true })] }); } catch {}
        ...
        await sendModLog(guild, logEmbed);
        const { getInvokeReply } = require('./invoke');
        const warnInvMsg = getInvokeReply(guild.id, 'warn', {
            targetName: target.user.username, targetId: target.id, targetMention: `<@${target.id}>`,
            modMention: `<@${authorId}>`, reason, guildName: guild.name, guildId: guild.id,
        });
        return ctx.reply(warnInvMsg || '👍');
    }
    ...
    if (command === 'note' || command === 'notes') {
        ...
        // .notes @user  or  .note @user <text>  (legacy)
    ...
    //  ALIASES
    // ════════════════════════════════════════════
    // .caselog → same as .case, .moderationhistory → same as .history
