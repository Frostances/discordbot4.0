 ...
 */
function getLiveTotal(userData, field) {
    const now = Date.now();
    let total = 0;
...
function get7DayTotal(userData, field) {
...
async function buildVoiceTimeLeaderboard(guild, db) {
    ...
    const sorted = Object.entries(guildStats)
        .map(([uid, data]) => {
            ...
            if (data.inVc && data.lastJoin) {
                total7d += (Date.now() - data.lastJoin);
            }
            return { uid, total7d };
        })
...
async function buildStreamsLeaderboard(guild, db) {
    ...
    const sorted = Object.entries(guildStats)
        .map(([uid, data]) => {
            ...
            // Add live session time for BOTH
            if (data.streaming && data.lastStreamJoin) {
                total7d += (Date.now() - data.lastStreamJoin);
            }
            if (data.cameraOn && data.lastCameraJoin) {
                total7d += (Date.now() - data.lastCameraJoin);
            }
            return { uid, total7d };
        })
        .filter(u => u.total7d > 0)
