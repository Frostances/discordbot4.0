...
async function handleBoosterRoleCommand(message, args) {
    ...
    if (sub === 'icon') {
        ...
        if (!iconArg) return message.reply(greedWarn(message.member, 'Provide an emoji or image URL. Usage: `.boosterrole icon <emoji>` or `.boosterrole icon remove`'));
        if (iconArg.toLowerCase() === 'remove') {
            try { await role.setIcon(null); db.set('boosterRoles', map); return message.reply(greedOk(message.member, `Role icon removed from <@&${role.id}>.`)); }
            catch (e) { return message.reply(greedWarn(message.member, `Failed to remove icon: ${e.message}`)); }
        }
        let iconValue = iconArg;
        const customEmojiMatch = iconArg.match(/<a?:(\w+):(\d+)>/);
        if (customEmojiMatch) iconValue = `https://cdn.discordapp.com/emojis/${customEmojiMatch[2]}.png?size=256`;
        try { await role.setIcon(iconValue); db.set('boosterRoles', map); return message.reply(greedOk(message.member, `Role icon updated for <@&${role.id}>.`)); }
