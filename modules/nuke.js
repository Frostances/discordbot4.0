...
function scheduleNuke(client, guildId, channelId, executorId, reason, fireAt) {
    ...
    const handle = setTimeout(async () => {
        ...
        try {
            ...
            await clone.send({ embeds: [base(COLORS.error)
                .setTitle('💣 Scheduled Nuke Executed')
                .setDescription(`This channel was nuked by <@${executorId}>.\nReason: ${reason}`)] })
                .catch(() => {});
        } catch (err) {
...
async function handleNuke(ctx, args, client) {
    ...
    try {
        ...
        await clone.send({ embeds: [base(COLORS.error)
            .setTitle('💣 Channel Nuked')
            .setDescription(`Nuked by <@${authorId}>.\n**Reason:** ${reason}`)] })
            .catch(() => {});
    } catch (err) {
