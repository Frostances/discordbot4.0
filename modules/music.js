...
async function cmdPreset(ctx, args) {
    ...
    if (isActive) {
        // Disable: reset all filters (simplest approach)
        removeActiveFilter(guildId, sub);
        ...
        return sendReply(ctx, { embeds: [successEmbed(`Disabled preset: **${sub}**`)] });
    } else {
        // Enable preset
        addActiveFilter(guildId, sub);
        const preset = PRESETS[sub];
        await player.setFilters(preset);
        return sendReply(ctx, { embeds: [successEmbed(`Enabled preset: **${sub}** 🎛️`)] });
    }
}
