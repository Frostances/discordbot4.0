...
async function handleInvokeCommand(message, args) {
  ...
  if (!subCmd || subCmd === 'list') {
    ...
    // Build one embed per command that has custom settings
    const pages = [];

    for (const cmd of VALID_COMMANDS) {
      const settings = invokes[cmd];
      const hasCustom = !!(settings?.message || settings?.dm);
      if (!hasCustom) continue; // Only show commands with custom settings
      ...
      if (settings?.message) {
        ...
        if (msgPreview.embeds?.[0]) {
          embed.addFields({
            name: '🎨 Embed Preview',
            value: `Title: ${msgPreview.embeds[0].data?.title || '(none)}\\nDescription: ${(msgPreview.embeds[0].data?.description || '(none)').substring(0, 100)}`,
            inline: false
          });
        }
      } else {
      ...
      if (settings?.dm) {
        ...
        if (dmPreview.embeds?.[0]) {
          embed.addFields({
            ...
            value: `Title: ${dmPreview.embeds[0].data?.title || '(none)}\\nDescription: ${(dmPreview.embeds[0].data?.description || '(none)').substring(0, 100)}`,
            inline: false
          });
        }
      } else {
    ...
    if (!pages.length) {
      return message.reply({
        embeds: [base(COLORS.primary).setTitle('📨 Invoke Messages').setDescription('No custom invoke messages set. Use `,invoke <cmd> message/dm <text>` to configure.\\n\\n**Available commands:**\\n' + VALID_COMMANDS.map(c => `\\`${c}\\``).join(', '))]
      });
    }

    // FIX: Use sendPaginatedEmbeds instead of non-existent sendPaginatedEmbeds from paginator
  ...
  const type = args[1]?.toLowerCase(); // 'message', 'dm', 'view', or 'reset'
