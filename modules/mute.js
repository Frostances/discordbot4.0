...
async function getOrCreateImageMuteRole(guild) {
 ...
 const role = await guild.roles.create({
   name: 'Image Muted', color: '#FF6B6B', permissions: [],
...
async function getOrCreateReactionMuteRole(guild) {
 ...
 const role = await guild.roles.create({
   name: 'Reaction Muted', color: '#FFB347', permissions: [],
...
// imute — mute images/attachments only (role-based)
async function handleIMute(ctx, args) {
 const { isStaffOrAdmin } = require('./helpers');
 if (!isStaffOrAdmin(ctx.member)) return ctx.reply({ content: '❌ No permission.', ephemeral: true });
 const target = ctx.mentions?.members?.first();
 if (!target) return ctx.reply({ content: '❌ Mention a user.', ephemeral: true });
 const reason = args.slice(1).join(' ') || 'Image mute';
 const authorId = ctx.author?.id || ctx.user?.id;

 const roleId = await getOrCreateImageMuteRole(ctx.guild);
 await target.roles.add(roleId, reason);

 const c = createCase(ctx.guild.id, { type: 'imute', targetId: target.id, executorId: authorId, reason });

 // Invoke DM
 try {
   const { sendInvokeDm } = require('./invoke');
   const { buildInvokeVars } = require('./moderation');
   const vars = buildInvokeVars(ctx, target, reason, null, c.id);
 ...
 try {
   const { sendInvokeReply } = require('./moderation');
   return sendInvokeReply(ctx, ctx.guild.id, 'imute', target, reason, null, c.id);
 } catch {
   return ctx.reply({ content: `🔇 Image/attachment mute applied to **${target.user.username}**.` });
 }
}
...
async function handleIUnmute(ctx, args) {
 ...
 const cfg = db.get('imuteConfig', {});
 if (cfg.roleId) await target.roles.remove(cfg.roleId, reason).catch(() => {});
...
// rmute — reaction mute (role-based)
async function handleRMute(ctx, args) {
 const { isStaffOrAdmin } = require('./helpers');
 if (!isStaffOrAdmin(ctx.member)) return ctx.reply({ content: '❌ No permission.', ephemeral: true });
 const target = ctx.mentions?.members?.first();
 if (!target) return ctx.reply({ content: '❌ Mention a user.', ephemeral: true });
 const reason = args.slice(1).join(' ') || 'Reaction mute';
 const authorId = ctx.author?.id || ctx.user?.id;

 const roleId = await getOrCreateReactionMuteRole(ctx.guild);
 await target.roles.add(roleId, reason);

 const c = createCase(ctx.guild.id, { type: 'rmute', targetId: target.id, executorId: authorId, reason });
 ...
 try {
   const { sendInvokeReply } = require('./moderation');
   return sendInvokeReply(ctx, ctx.guild.id, 'rmute', target, reason, null, c.id);
 } catch {
   return ctx.reply({ content: `🔇 Reaction mute applied to **${target.user.username}**.` });
 }
}
...
async function handleSetupIMute(ctx, args) {
 ...
 return ctx.reply({
   embeds: [base(COLORS.success).setTitle('🖼️ Image Mute System Set Up')
     ...
     .addFields(
       { name: 'Role Name', value: role?.name || 'Image Muted', inline: true },
...
async function handleSetupRMute(ctx, args) {
 ...
 return ctx.reply({
   embeds: [base(COLORS.success).setTitle('😶 Reaction Mute System Set Up')
     ...
     .addFields(
       { name: 'Role Name', value: role?.name || 'Reaction Muted', inline: true },
