            ...
            for (const seg of segments) {
              ...
              switch (key) {
                ...
                case 'color':
                  ...
                  break;
                case 'footer': {
                  const fp = value.split(/\s*&&\s*/);
                  embed.setFooter({ text: fp[0] || '', iconURL: fp[1] || undefined });
                  break;
                }
                case 'author': {
                  const ap = value.split(/\s*&&\s*/);
                  embed.setAuthor({ name: ap[0] || '', iconURL: ap[1] || undefined });
                  break;
                }
                case 'thumbnail':
                ...
                case 'timestamp':
                  ...
                  break;
                case 'field': {
                  const fm = value.match(/^(.+?)\s*&&\s*(.+?)(?:\s+(inline))?$/i);
                  if (fm) {
                    embed.addFields({ name: fm[1].trim(), value: fm[2].trim(), inline: !!fm[3] });
                  } else {
                    const p = value.split(/\s*&&\s*/);
                    embed.addFields({ name: p[0] || '\u200B', value: p[1] || '\u200B', inline: false });
                  }
                  break;
                }
              }
          ...
          function getInvokeDm(guildId, command, vars) {
            ...
            // No custom DM = disabled (no default DMs)
            if (!custom || !custom.trim()) return null;
            return parseInvokeMessage(custom, vars);
          }
          ...
          async function handleInvokeCommand(message, args) {
            ...
            if (type === 'view') {
              ...
              if (settings.dm) {
                const dmPreview = parseInvokeMessage(settings.dm, previewVars);
                embed.addFields({ name: '\uD83D\uDCE9 DM (enabled)', value: dmPreview.content || '(embed)', inline: false });
              } else {
                embed.addFields({ name: '\uD83D\uDCE9 DM', value: '\u274C Disabled (no default)', inline: false });
              }
            ...
            if (subType === 'reset') {
              ...
              if (type === 'message') {
                delete invokes[subCmd].message;
              }
              if (type === 'dm') {
                delete invokes[subCmd].dm;
              }
              if (!invokes[subCmd].message && !invokes[subCmd].dm) delete invokes[subCmd];
              db.set('invokeMessages', invokes);

              const typeLabel = type === 'message' ? 'Public Reply' : 'DM';
